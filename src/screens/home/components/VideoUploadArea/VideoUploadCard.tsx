...
export function VideoUploadCard() {
  ...
  return (
    <View style={tw` h-[218px] rounded-[10px] overflow-hidden`}>
      {!uploaded ? (
        <View style={tw`flex-1  rounded-[10px] border-2 border-[#E9ECEF] border-dashed overflow-hidden`}>
          <BlurView
            blurType="dark"
            blurAmount={10}
          ...
          <View style={tw`justify-center items-center flex-1 pt-[40px]`}>
            {!isLoading ? <UploadIcon width={45} height={45} /> : <ActivityIndicator size="large" />}
            <View style={tw`mt-3`}>
              <Text variant={TextVariant.Body1Regular}>No Data to Report. Get Started!</Text>
